...
function loadData(startDate = '', endDate = '') {
  $(document).ready(function () {
    ...
    $('#data-table').DataTable({
      ajax: {
        url: baseUrl + '/fund-distribution/list_data',
        type: 'GET',
        data: {
...
function fetchTotalMonthlyIncome(callback, branch_id = null) {
  $.ajax({
      url: baseUrl + '/fund-distribution/get-income',
      type: 'GET',
...
function updateCreditBasedOnProgram(selectedData) {
  ...
  let branch_id = selectedData.branch_id || null;
...
function updateCreditBasedOnProgramEdit(percentage, branches_id = null) {
  if (percentage > 0) {
    fetchTotalMonthlyIncome(function (totalIncome) {
    ...
    $('#EditCredit').prop('readonly', false);
  }
}

function loadBeneficaries(callback) {
  $.ajax({
      url: '/super-admin/beneficaries/list_data',
      type: 'GET',
      success: function (response) {
...
function detail(id) {
  $.ajax({
      url: baseUrl + '/fund-distribution/get-detail',
      type: 'GET',
      data: { id: id },
      dataType: 'json',
      success: function (response) {
...
handleAddForm(
  '#formCreate',
  '/super-admin/fund-distribution/save',
  'modalCreate',
...
$(document).on('click', '#btnEdit', function () {
    ...
    updateCreditBasedOnProgramEdit(percentage, branch_id);
...
handleEditForm(
  '#formEdit',
  '/super-admin/fund-distribution/update',
  'modalEdit',
...
$(document).on('click', '#btnDelete', function () {
  ...
  handleDelete('/super-admin/fund-distribution/delete', id, function () {
    loadData();
  });
});