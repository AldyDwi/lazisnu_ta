...
var prefix_folder_admin = 'super_admin/';
...
// Create Data
function handleAddForm(formId, url, modalId, fields, loadData, customFormData = null) {
  $(formId).off('submit').on('submit', function (e) {
    e.preventDefault();
    let formData = customFormData || new FormData($(formId)[0]);
    NProgress.start();

    let fileInput = $(formId).find('input[type="file"]');
    if (fileInput.length > 0 && pond.getFiles().length > 0) {
...
// Update Data
function handleEditForm(formId, url, modalId, fields, loadData, customFormData = null) {
  $(formId).off('submit').on('submit', function (e) {
    e.preventDefault();
    let formData = customFormData || new FormData($(formId)[0]);
    let id = $('#EditId').val();
    ...
    let fileInput = $(formId).find('input[type="file"]');
    if (fileInput.length > 0 && pondEdit.getFiles().length > 0) {
      formData.append('image', pondEdit.getFiles()[0].file);
    }
